 ...
 */
export async function saveManifest(db, files, manifestTransactionId = null) {
  try {
    if (!db) {
    ...
    // Save manifest transaction ID if provided (for redirect endpoint)
    if (manifestTransactionId) {
      manifestData.manifestTransactionId = manifestTransactionId;
      manifestData.websiteUrl = `https://arweave.net/${manifestTransactionId}/index.html`;
      manifestData.manifestUrl = `https://arweave.net/${manifestTransactionId}`;
    }
    
    await manifestRef.set(manifestData, { merge: false });
    
    console.log(`[DeploymentTracker] ✅ Saved manifest with ${Object.keys(files).length} files`);
    if (manifestTransactionId) {
      console.log(`[DeploymentTracker] ✅ Saved manifest transaction ID: ${manifestTransactionId}`);
    }
  } catch (error) {
