...
export async function syncFirebaseToWebsiteJSON(db, websiteDir = 'website') {
  try {
    ...
    // The websiteDir parameter can be either:
    // - A relative path like 'website' (will be resolved from cwd)
    // - An absolute path like '/tmp/website' (used as-is)
    let websitePath;
    if (path.isAbsolute(websiteDir)) {
      websitePath = websiteDir;
    } else {
      websitePath = path.join(process.cwd(), websiteDir);
    }
    
    const artistsJsonPath = path.join(websitePath, 'artists.json');
    console.log(`[WebsiteSync] Writing to: ${artistsJsonPath}`);
