...
export async function syncFirebaseToWebsiteJSON(db, websiteDir = 'website') {
  try {
    ...
    if (!artistsDoc.exists) {
      console.warn('[WebsiteSync] No artists found in Firebase');
      return {
        success: false,
        error: 'No artists found in Firebase',
        artistsCount: 0
      };
    }
    ...
    if (firebaseArtists.length === 0) {
      console.warn('[WebsiteSync] Artists array is empty in Firebase');
      return {
        success: false,
        error: 'Artists array is empty',
        artistsCount: 0
      };
    }
