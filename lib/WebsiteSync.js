...
export async function syncFirebaseToWebsiteJSON(db, websiteDir = 'website') {
  try {
    ...
    // Use /tmp directory in Vercel for writable operations
    ...
    if (isVercelProduction) {
      // Use /tmp in Vercel (writable directory)
      websitePath = '/tmp/website';
      artistsJsonPath = path.join(websitePath, 'artists.json');
      console.log('[WebsiteSync] Using /tmp directory for Vercel production');
    } else {
      websitePath = path.join(process.cwd(), websiteDir);
      artistsJsonPath = path.join(websitePath, 'artists.json');
    }
