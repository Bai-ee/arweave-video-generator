...
/**
 * Rewrite a raw Arweave tx URL to a gateway that honors HTTP Range requests, so the
 * audio player can scrub. The canonical arweave.net gateway currently returns 200
 * (no Range / no Content-Length) for bundled data-items, which leaves the media
 * element unseekable and freezes on seek. Only raw arweave.net tx URLs are rewritten;
 * any other host (ardrive.net, app.ardrive.io viewer links, etc.) passes through
 * untouched. Canonical URLs stay stored as-is — this is a presentation-layer swap, so
 * changing gateways later is a one-line edit here + redeploy.
 */
const PLAYBACK_GATEWAY = 'https://ar-io.dev';
function toPlaybackGateway(url) {
  if (!url) return url;
  const m = url.match(/^https?:\/\/(?:[a-z0-9-]+\.)?arweave\.net\/([A-Za-z0-9_-]{43})(?:[/?#].*)?$/);
  return m ? `${PLAYBACK_GATEWAY}/raw/${m[1]}` : url;
}
...
function generatePlaylistItems(artist, mixes) {
  // SVG icons for play/pause hover state
  const playSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.54,9,8.88,3.46a3.42,3.42,0,0,0-5.13,3V17.58A3.42,3.42,0,0,0,7.17,21a3.43,3.43,0,0,0,1.71-.46L18.54,15a3.42,3.42,0,0,0,0-5.92Zm-1,4.19L7.88,18.81a1.44,1.44,0,0,1-1.42,0,1.42,1.42,0,0,1-.71-1.23V6.42a1.42,1.42,0,0,1,.71-1.23A1.51,1.51,0,0,1,7.17,5a1.54,1.54,0,0,1,.71.19l9.66,5.58a1.42,1.42,0,0,1,0,2.46Z"/></svg>';
  const pauseSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16,2a3,3,0,0,0-3,3V19a3,3,0,0,0,6,0V5A3,3,0,0,0,16,2Zm1,17a1,1,0,0,1-2,0V5a1,1,0,0,1,2,0ZM8,2A3,3,0,0,0,5,5V19a3,3,0,0,0,6,0V5A3,3,0,0,0,8,2ZM9,19a1,1,0,0,1-2,0V5A1,1,0,0,1,9,5Z"/></svg>';
  ...
  return mixes.map(mix => {
    ...
    <li class="single-item">
      <a data-playlist data-title="${escapeHtml(mix.mixTitle)}" data-artist="${escapeHtml(artist.artistName)}" data-img="${imgSrc}" href="${toPlaybackGateway(mix.mixArweaveURL) || '#'}" class="single-item__cover">
        <img src="${imgSrc}" alt="">
        ${playSvg}
        ${pauseSvg}
      </a>
...
function generateFeaturedTrack(artist) {
  ...
  return `
<li class="single-item">
  <a data-link data-title="${escapeHtml(firstMix.mixTitle)}" data-artist="${escapeHtml(artist.artistName)}" data-img="${imgSrc}" href="${toPlaybackGateway(firstMix.mixArweaveURL) || '#'}" class="single-item__cover">
...
function generateArtistPages(artistsJsonPath, templatePath, outputDir) {
  ...
  artists.forEach(artist => {
    ...
    // Get the artist image - could be full Arweave URL or local path
    const artistImage = artist.artistImageFilename || '';
    const isFullUrl = artistImage.startsWith('http');
    
    // [ARTIST_COVER] - handle both full URLs and local paths
    // Template uses patterns like: img/covers/[ARTIST_COVER].jpg
    // For full URLs, we need to replace the whole img/covers/[ARTIST_COVER].jpg pattern
    // For local paths, extract filename without extension
    let coverFilename = '';
    if (artistImage) {
      if (!isFullUrl) {
        // Extract just the filename without extension
        // e.g., "img/artists/acidman.jpg" -> "acidman"
        const filename = path.basename(artistImage);
        coverFilename = filename.replace(/\.(jpg|jpeg|png|gif|webp)$/i, '');
      }
    }
    
    // Replace cover image patterns in template
    if (isFullUrl) {
      // Full URL - replace entire img/covers/[ARTIST_COVER].jpg pattern with the URL
      html = html.replace(/img\/covers\/\[ARTIST_COVER\]\.jpg/g, artistImage);
      html = html.replace(/\.\.\/img\/covers\/\[ARTIST_COVER\]\.jpg/g, artistImage);
      // Also replace any [ARTIST_COVER] that might be used elsewhere
      html = html.replace(/\[ARTIST_COVER\]/g, artistImage);
    } else {
      // Local path - use filename pattern
    ...
    // Only do this if we have a local path (not full URL)
    if (coverFilename && !isFullUrl) {
