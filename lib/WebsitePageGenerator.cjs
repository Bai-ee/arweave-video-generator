...
function generateArtistPages(artistsJsonPath, templatePath, outputDir) {
  ...
  artists.forEach(artist => {
    ...
    // Fix artist cover image paths - template uses img/covers/ but artist images are in img/artists/
    // Replace img/covers/[ARTIST_COVER].jpg with img/artists/[ARTIST_COVER].jpg
    if (coverFilename && !coverFilename.startsWith('http')) {
      // Replace all instances of img/covers/[coverFilename].jpg with img/artists/[coverFilename].jpg
      const coverPattern = new RegExp(`img/covers/${coverFilename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\.jpg`, 'g');
      html = html.replace(coverPattern, `img/artists/${coverFilename}.jpg`);
    }
