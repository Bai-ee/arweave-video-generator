...
function generateArtistPages(artistsJsonPath, templatePath, outputDir) {
  ...
  artists.forEach(artist => {
    ...
    // [ARTIST_COVER] - use absolute path from manifest root
    let coverPath = artist.artistImageFilename && artist.artistImageFilename.startsWith('http') 
      ? artist.artistImageFilename 
      : artist.artistImageFilename || '';
    // Ensure path is absolute from root (img/artists/filename)
    if (coverPath && !coverPath.startsWith('http') && !coverPath.startsWith('img/')) {
      coverPath = `img/artists/${coverPath}`;
    } else if (coverPath && !coverPath.startsWith('http')) {
      // Already has img/ prefix, use as-is
    }
    html = html.replace(/\[ARTIST_COVER\]/g, coverPath.replace('img/artists/', '').replace('img/', ''));
    // [MIX_COUNT]
