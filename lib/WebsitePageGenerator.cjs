...
function generateFeaturedTrack(artist) {
  ...
  return `
<li class="single-item">
  <a data-link data-title="${escapeHtml(firstMix.mixTitle)}" data-artist="${escapeHtml(artist.artistName)}" data-img="${fullImgSrc}" href="${firstMix.mixArweaveURL || '#'}" class="single-item__cover">
...
function generatePages(artistsJsonPath = null, outputDir = null) {
  ...
  // Use outputDir for index.html if provided (needed for Vercel /tmp writes)
