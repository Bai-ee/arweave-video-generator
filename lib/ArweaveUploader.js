...
export async function uploadToArweave(fileData, fileName, options = {}) {
  try {
    ...
    console.log('[ArweaveUploader] File size:', fileData.length, 'bytes');
    console.log('[ArweaveUploader] Tags count:', tags.length);
    console.log('[ArweaveUploader] Wallet address:', walletAddress || 'Not set');
    let turboResult;
    try {
      ...
      console.error('[ArweaveUploader] Full error object keys:', Object.keys(uploadError));
      if (uploadError.cause) {
        console.error('[ArweaveUploader] Error cause:', uploadError.cause);
      }
      ...
      if (uploadError.response) {
        ...
        console.error('[ArweaveUploader] Response URL:', uploadError.response.config?.url || 'Unknown');
        console.error('[ArweaveUploader] Response headers:', JSON.stringify(uploadError.response.headers, null, 2));
        console.error('[ArweaveUploader] Response data type:', typeof responseData);
        console.error('[ArweaveUploader] Response data (full):', typeof responseData === 'string' ? responseData : JSON.stringify(responseData, null, 2));
...
/**
 * Upload file as atomic asset with ANS-110 tags
 * @param {Buffer|Uint8Array} fileData - File data as buffer
 * @param {string} fileName - Name of the file
 * @param {Object} atomicMetadata - ANS-110 metadata object
 * @param {Array} additionalTags - Additional tags to include
 * @returns {Promise<Object>} Upload result with transaction ID and URLs
 */
export async function uploadAtomicAsset(fileData, fileName, atomicMetadata, additionalTags = []) {
  try {
    // Validate environment variables
    if (!process.env.ARWEAVE_WALLET_JWK) {
      throw new Error('ARWEAVE_WALLET_JWK environment variable is required');
    ...
    // Parse wallet JWK (same logic as existing uploadToArweave)
    let walletJwk;
    let jwkString = process.env.ARWEAVE_WALLET_JWK;
    ...
    console.log('[ArweaveUploader] Starting atomic asset upload...');
    console.log(`[ArweaveUploader] File: ${fileName}`);
    console.log(`[ArweaveUploader] Size: ${fileData.length} bytes`);
    ...
    // Build tags from atomic metadata
    const tags = [];
    
    // Add all atomic metadata fields as tags
    Object.entries(atomicMetadata).forEach(([key, value]) => {
    ...
    // Add drive/folder IDs if provided
    if (driveId) {
      tags.push({ name: 'Drive-ID', value: driveId });
    }
    if (folderId) {
    ...
    console.log(`[ArweaveUploader] Tags count: ${tags.length}`);
    console.log(`[ArweaveUploader] Contract-Src: ${atomicMetadata['Contract-Src'] || 'Not set'}`);

    // Upload to Turbo
    console.log('[ArweaveUploader] ⚡ Uploading atomic asset to Turbo...');
    ...
    try {
      ...
      if (uploadError.response) {
        const status = uploadError.response.status;
        const statusText = uploadError.response.statusText;
        throw new Error(`Arweave upload failed with HTTP ${status}: ${statusText}`);
      }
      throw new Error(`Turbo upload failed: ${uploadError.message}`);
    }
    ...
    console.log('[ArweaveUploader] ✅ Atomic asset upload successful!');
    console.log(`[ArweaveUploader] Transaction ID: ${turboResult.id}`);
    ...
    const result = {
      ...
      fileName: fileName,
      fileSize: fileData.length,
      contentType: atomicMetadata['Content-Type'] || 'video/mp4',
      atomicAsset: true,
      metadata: atomicMetadata,
      uploadDate: new Date().toISOString(),
      note: 'Atomic asset uploaded successfully. Transaction is pending blockchain confirmation (typically 2-10 minutes).'
    };

    return result;

  } catch (error) {
    console.error('[ArweaveUploader] ❌ Atomic asset upload failed:', error.message);
    return {
      success: false,
      error: error.message,
      fileName: fileName,
