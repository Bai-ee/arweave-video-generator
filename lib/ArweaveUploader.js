...
export async function uploadToArweave(fileData, fileName, options = {}) {
  try {
    ...
    try {
      ...
      walletJwk = JSON.parse(process.env.ARWEAVE_WALLET_JWK);
    } catch (parseError) {
      // If parsing fails, try to clean up the string
      // Remove any newlines and extra whitespace
      const cleaned = process.env.ARWEAVE_WALLET_JWK
        .replace(/\n/g, '')
        .replace(/\r/g, '')
        .replace(/\s+/g, ' ')
        .trim();
      ...
      try {
        ...
        console.error('[ArweaveUploader] JSON Parse Error:', parseError.message);
        console.error('[ArweaveUploader] JWK length:', process.env.ARWEAVE_WALLET_JWK.length);
        console.error('[ArweaveUploader] JWK preview (first 100 chars):', process.env.ARWEAVE_WALLET_JWK.substring(0, 100));
        throw new Error(`Failed to parse ARWEAVE_WALLET_JWK: ${parseError.message}. Please ensure the JSON is properly formatted as a single-line string in Vercel.`);
      }
    }
