...
export async function deployWebsiteToArweave(websiteDir = 'website', db = null) {
  try {
    ...
    // Verify critical files exist (banner image)
    const criticalFiles = ['img/covers/ue_banner.jpg'];
    ...
    for (const criticalFile of criticalFiles) {
      ...
      if (!exists) {
        ...
        if (fileExists) {
          ...
          console.error(`[WebsiteDeployer] ❌ Critical file ${criticalFile} is missing! This will cause broken images on the deployed website.`);
        }
      } else {
    ...
    // Verify banner image is in manifest
    const bannerInManifest = uploadResults.some(r => r.path === 'img/covers/ue_banner.jpg' || r.path.replace(/\\/g, '/') === 'img/covers/ue_banner.jpg');
    if (!bannerInManifest) {
      console.error(`[WebsiteDeployer] ❌ WARNING: img/covers/ue_banner.jpg is NOT in the manifest! This will cause a broken image.`);
      console.error(`[WebsiteDeployer] Available img/covers files:`, uploadResults.filter(r => r.path.includes('img/covers')).map(r => r.path));
    } else {
      console.log(`[WebsiteDeployer] ✅ Banner image (img/covers/ue_banner.jpg) is in manifest`);
    }
