...
export async function deployWebsiteToArweave(websiteDir = 'website', db = null) {
  try {
    ...
    for (const criticalFile of criticalFiles) {
      const exists = allFiles.some(f => f === criticalFile || f.replace(/\\/g, '/') === criticalFile);
      if (!exists) {
        const criticalFilePath = path.join(fullWebsitePath, criticalFile);
        const fileExists = await fs.pathExists(criticalFilePath);
        if (fileExists) {
          console.warn(`[WebsiteDeployer] ⚠️ Critical file ${criticalFile} exists on disk but not in file list - may need to be uploaded`);
        } else {
          console.error(`[WebsiteDeployer] ❌ Critical file ${criticalFile} is missing! This will cause broken resources on the deployed website.`);
        }
      } else {
