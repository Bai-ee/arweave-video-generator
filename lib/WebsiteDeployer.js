...
export async function collectWebsiteFiles(websiteDir) {
  ...
  async function walkDir(dir, baseDir = websiteDir) {
    try {
      ...
      console.error(`[collectWebsiteFiles] Error walking directory ${dir}:`, error.message);
      throw error;
    }
  }
...
export async function deployWebsiteToArweave(websiteDir = 'website', db = null) {
  try {
    ...
    // Verify critical files exist (banner image, logo, font)
    const criticalFiles = [
      'img/covers/ue_banner.jpg',
      'img/loge_horiz.png',
      'fonts/IBM_Plex_Mono,Rationale,Shantell_Sans/Rationale/Rationale-Regular.ttf'
    ];
    const allFiles = [...changedFiles.map(f => f.path), ...unchangedFiles.map(f => f.path)];
    console.log(`[WebsiteDeployer] Checking ${criticalFiles.length} critical files against ${allFiles.length} collected files...`);
    for (const criticalFile of criticalFiles) {
      const exists = allFiles.some(f => {
        const normalized = f.replace(/\\/g, '/');
        return normalized === criticalFile;
      });
      if (!exists) {
        const criticalFilePath = path.join(fullWebsitePath, criticalFile);
        const fileExists = await fs.pathExists(criticalFilePath);
        if (fileExists) {
          console.warn(`[WebsiteDeployer] ⚠️ Critical file ${criticalFile} exists on disk at ${criticalFilePath} but not in file list`);
          console.warn(`[WebsiteDeployer] File collection may have failed - checking similar paths...`);
          const similarFiles = allFiles.filter(f => f.includes(criticalFile.split('/').pop()));
          if (similarFiles.length > 0) {
            console.warn(`[WebsiteDeployer] Found similar files: ${similarFiles.slice(0, 3).join(', ')}`);
          }
        } else {
          console.error(`[WebsiteDeployer] ❌ Critical file ${criticalFile} is missing from disk at ${criticalFilePath}!`);
          console.error(`[WebsiteDeployer] This will cause broken resources on the deployed website.`);
        }
      } else {
        console.log(`[WebsiteDeployer] ✅ Critical file ${criticalFile} found in deployment list`);
      }
    }
    ...
    // Verify critical resources are in manifest
    ...
    for (const resource of criticalResources) {
      const inManifest = uploadResults.some(r => {
        const normalizedPath = r.path.replace(/\\/g, '/');
        return normalizedPath === resource.path;
      });
      if (!inManifest) {
        console.error(`[WebsiteDeployer] ❌ WARNING: ${resource.name} (${resource.path}) is NOT in the manifest! This will cause a broken resource.`);
        const relatedFiles = uploadResults.filter(r => r.path.includes(resource.path.split('/')[0]));
        if (relatedFiles.length > 0) {
          console.error(`[WebsiteDeployer] Available ${resource.path.split('/')[0]} files:`, relatedFiles.slice(0, 5).map(r => r.path));
        }
      } else {
        console.log(`[WebsiteDeployer] ✅ ${resource.name} (${resource.path}) is in manifest`);
      }
    }
    ...
    // Normalize paths to use forward slashes for Arweave manifest
    ...
    // Log manifest summary for debugging
    console.log(`[WebsiteDeployer] Manifest contains ${Object.keys(manifest.paths).length} paths`);
    const criticalPaths = ['img/covers/ue_banner.jpg', 'img/loge_horiz.png', 'css/main.css', 'index.html'];
    console.log('[WebsiteDeployer] Critical paths in manifest:');
    criticalPaths.forEach(criticalPath => {
      if (manifest.paths[criticalPath]) {
        console.log(`  ✅ ${criticalPath} -> ${manifest.paths[criticalPath].id}`);
      } else {
        console.log(`  ❌ ${criticalPath} -> MISSING`);
      }
    });
