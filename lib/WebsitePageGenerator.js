...
function generatePlaylistItems(artist, mixes) {
  return mixes.map(mix => {
    // Use mixImageFilename as-is if it starts with 'http', else prefix with '../'
    const imgSrc = mix.mixImageFilename && mix.mixImageFilename.startsWith('http') 
...
function generateArtistCard(artist) {
  const imgSrc = artist.artistImageFilename && artist.artistImageFilename.startsWith('http')
    ? artist.artistImageFilename
    : `img/artists/${artist.artistImageFilename || ''}`;
...
function generateFeaturedTrack(artist) {
  ...
  const imgSrc = firstMix.mixImageFilename && firstMix.mixImageFilename.startsWith('http')
    ? firstMix.mixImageFilename
    : (firstMix.mixImageFilename || artist.artistImageFilename || '');
  const fullImgSrc = imgSrc.startsWith('http') ? imgSrc : `img/${imgSrc}`;
  
  return `
<li class="single-item">
  <a data-link data-title="${escapeHtml(firstMix.mixTitle)}" data-artist="${escapeHtml(artist.artistName)}" data-img="${fullImgSrc}" href="${toPlaybackGateway(firstMix.mixArweaveURL) || '#'}" class="single-item__cover">
  <img style="max-width:300px; max-height:300px;" src="${fullImgSrc}" alt="${escapeHtml(firstMix.mixTitle)}">
...
function generateArtistPages(artistsJsonPath, templatePath, outputDir) {
  ...
  artists.forEach(artist => {
    ...
    // [ARTIST_COVER] (strip 'img/' prefix for covers, but keep for artists)
    let coverPath = artist.artistImageFilename && artist.artistImageFilename.startsWith('http') 
      ? artist.artistImageFilename 
      : `../${artist.artistImageFilename || ''}`;
    html = html.replace(/\[ARTIST_COVER\]/g, coverPath.replace('img/', 'artists/'));
    // [MIX_COUNT]
...
function generatePages(artistsJsonPath = null, outputDir = null) {
  ...
  const defaultOutputDir = baseDir;
  const defaultIndexHtml = path.join(baseDir, 'index.html');

  const artistsJson = artistsJsonPath || defaultArtistsJson;
  const template = defaultTemplate;
  const output = outputDir || defaultOutputDir;
  const indexHtml = defaultIndexHtml;
