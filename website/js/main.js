$(document).ready(function () {
	...
	<div class="plyr__controls">
		<div class="plyr__actions">
			...
			<button type="button" class="plyr__control" data-plyr="play">
				<svg class="icon--pressed" role="presentation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16,2A3,3,0,0,0,13,5V19a3,3,0,0,0,6,0V5A3,3,0,0,0,16,2Zm1,17a1,1,0,0,1-2,0V5a1,1,0,0,1,2,0ZM8,2A3,3,0,0,0,5,5V19a3,3,0,0,0,6,0V5A3,3,0,0,0,8,2ZM9,19a1,1,0,0,1-2,0V5A1,1,0,0,1,9,5Z"></path></svg>
				<svg class="icon--not-pressed" role="presentation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.54,9,8.88,3.46a3.42,3.42,0,0,0-5.13,3V17.58A3.42,3.42,0,0,0,7.17,21a3.43,3.43,0,0,0,1.71-.46L18.54,15a3.28,3.28,0,0,0,1.68-.47,3.24,3.24,0,0,0,1.61.43,3.38,3.38,0,0,0,1.72,3V6.42A3.38,3.38,0,0,0,20.28,3.43ZM16.83,9.17a1,1,0,1,0-1.42,1.42A2,2,0,0,1,16,12a2,2,0,0,1-.71,1.53,1,1,0,0,0-.13,1.41,1,1,0,0,0,1.41.12A4,4,0,0,0,18,12,4.06,4.06,0,0,0,16.83,9.17Z"/></svg>
			</button>
	...
	// Initialize player with default content on page load
	$(document).ready(function() {
		// Check if we're on an artist page (not index page) by looking for specific elements
		if ((window.location.pathname !== '/' && window.location.pathname !== '/index.html')) {
			// Find the first track link (either data-link or data-playlist)
			let firstTrack = $('a[data-link], a[data-playlist]').first();
			if (firstTrack.length > 0) {
				// Set it as active and initialize the player with its content
				firstTrack.addClass('active');
				
				// Set the player source and metadata
				audio[0].src = firstTrack.attr('href');
				let title = firstTrack.data('title');
				let artist = firstTrack.data('artist');
				let img = firstTrack.data('img');
				
				if (title) $('.player__title').text(title);
				if (artist) $('.player__artist').text(artist);
				if (img) $('.player__cover img').attr('src', img);
	...
	function run(link, player){
		if ($(link).hasClass('play')) {
			...
			$('a[data-link], a[data-playlist]').removeClass('active');
			$('a[data-link], a[data-playlist]').removeClass('pause');
			$('a[data-link], a[data-playlist]').removeClass('play');
	...
	if ($('.main__list--playlist').length) {
		...
		playlist.find('a[data-playlist]').on('click', function(e){
			e.preventDefault();
			e.stopPropagation();
			console.log('Playlist link click prevented');
			let link = $(this);
			current = link.parent().index();
			run(link, audio[0]); // Use the same run function for consistency
		});
		...
		player.on('ended', event => {
			...
			let link = playlist.find('a[data-playlist]').eq(current);
			run(link, audio[0]);
		});
